...
class HospitalViewSet(viewsets.ModelViewSet):
    ...
    def closest_hospitals(self, request):
        """Get hospitals that are at least three kilometers or less from a users location"""
        ...
        if longitude and latitude:
            ...
            closest_hospitals = Hospital.objects.filter(
                geometry_field__distance_lte=(user_location, D(km=3))
            )
            serializer = self.get_serializer(closest_hospitals, many=True)
